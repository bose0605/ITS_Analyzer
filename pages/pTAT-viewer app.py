...
# ==== タブのラベルと対応するヘッダー ====
tab_labels = ["Frequency", "CPU temp", "IA-clip reason","GT-clip reason"]
tab_headers = {
...
with tabs[0]:
    ...
    # Frequency タブ専用の処理
    frequency_cols = [col for col in df.columns if "cpu" in col.lower() and "freq" in col.lower()]
    if frequency_cols:
        fig_freq = go.Figure()
...
with tabs[1]:
    ...
    # CPU温度の列を抽出（CPUxx-DTS のような名前の列）
    temp_cols = [col for col in df.columns if re.search(r"CPU\d+-DTS", col)]
